...
export function EventCountdown({ eventDate }: EventCountdownProps) {
  ...
  return (
    <div className="text-center p-4 rounded-lg bg-gray-100 dark:bg-gray-800 shadow-md">
      <h3 className="text-lg font-semibold mb-2 dark:text-white">Tiempo para el Check-in:</h3>
      {timeLeft ? (
        <div className="grid grid-flow-col gap-5 text-center auto-cols-max justify-center">
            <div className="flex flex-col">
                <span className="font-mono text-3xl">{String(timeLeft.days).padStart(2, '0')}</span>
                días
            </div>
            <div className="flex flex-col">
                <span className="font-mono text-3xl">{String(timeLeft.hours).padStart(2, '0')}</span>
                horas
            </div>
            <div className="flex flex-col">
                <span className="font-mono text-3xl">{String(timeLeft.minutes).padStart(2, '0')}</span>
                min
            </div>
            <div className="flex flex-col">
                <span className="font-mono text-3xl">{String(timeLeft.seconds).padStart(2, '0')}</span>
                seg
            </div>
        </div>
      ) : (
