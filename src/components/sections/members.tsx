...
import ActiveMemberCard from '../members/ActiveMemberCard';
import {Member} from "@/types/member";
import ActiveMemberModal from '../members/ActiveMemberModal';
        ...
        <div id="members" className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {members
                    ...
                    .map((member) => (
                        <ActiveMemberCard key={member.id} member={member} onClick={() => handleMemberClick(member)}/>
                    ))}
                {/* Inactive Members Card */}
                <Link href="/inactive-members" className="group">
                    <Card
                        className="group cursor-pointer transform transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:-translate-y-4 rounded-2xl overflow-hidden border-2 border-dashed border-gray-300 bg-gray-50 hover:bg-gray-100 shadow-lg aspect-[3/4] relative mx-auto max-w-sm w-full"
                    >
                        <CardContent className="p-0 h-full relative flex flex-col items-center justify-end text-center">
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                <div
                                    className="w-20 h-20 rounded-full bg-gray-200 flex items-center justify-center group-hover:bg-gray-300 transition-colors duration-300 mb-4">
                            ...
                            <div
                                className="absolute inset-0 bg-gradient-to-t from-gray-200/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"/>
                        </CardContent>
                    </Card>
                </Link>
            </div>
